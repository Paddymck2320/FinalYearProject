...
def game_data(game_df, game_stats):
    try:
        ...
        away_stats_df = game_stats.dataframe[['away_first_downs', 'away_fourth_down_attempts',
                                              ...
                                              'away_yards_lost_from_sacks']].reset_index().drop(columns='index').rename(
            columns={
        ...
        home_stats_df = game_stats.dataframe[['home_first_downs', 'home_fourth_down_attempts',
                                              ...
                                              'home_yards_lost_from_sacks']].reset_index().drop(columns='index').rename(
            columns={
...
def agg_weekly_data(schedule_df, weeks_games_df, current_week, weeks):
    ...
    for w in range(1, len(weeks)):
        ...
        agg_weekly_df = weeks_games_df[weeks_games_df.week < weeks[w]].drop(
            columns=['score', 'week', 'game_won', 'game_lost']).groupby(
...
def get_elo():
    elo_df = pd.read_csv('nfl_data/nfl_elo_latest.csv')
    elo_df = elo_df.drop(
        columns=['season', 'neutral', 'playoff', 'elo1_pre', 'elo2_pre', 'elo_prob1', 'elo_prob2', 'elo1_post',
                 'elo2_post',
                 'qbelo1_pre', 'qbelo2_pre', 'qb1', 'qb2', 'qb1_value_pre', 'qb2_value_pre', 'qb1_adj', 'qb2_adj',
    ...
    elo_df['team1'] = elo_df['team1'].replace(['KC', 'JAX', 'CAR', 'BAL', 'BUF', 'MIN', 'DET', 'ATL', 'NE', 'WSH',
                                              ...
                                              ['kan', 'jax', 'car', 'rav', 'buf', 'min', 'det', 'atl', 'nwe', 'was',
                                               ...
                                               'tam', 'mia', 'pit', 'phi', 'gnb', 'chi', 'dal', 'crd', 'sdg', 'htx',
                                               'sea', 'rai'])
...
def merge_rankings(agg_games_df, elo_df):
    agg_games_df = pd.merge(agg_games_df, elo_df, how='inner', left_on=['home_abbr', 'away_abbr'],
                            right_on=['team1', 'team2']).drop(columns=['date', 'team1', 'team2'])
