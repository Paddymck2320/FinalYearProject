...
def eloEval(eloA, eloB, scoreA, scoreB, streakA, streakB):
    ...
    if scoreA > scoreB:
        if streakA >= 0:
            STREAK_FACTOR_A = (-0.5) * STREAK_LIMIT + (1.5 * STREAK_LIMIT) / (
                (1.0 + math.exp((-1) * RISING_STREAK_CONSTANT * abs(streakA))))
        else:
            STREAK_FACTOR_A = 1
        if streakB < 0:
            STREAK_FACTOR_B = (-0.5) * STREAK_LIMIT + (1.5 * STREAK_LIMIT) / (
                (1.0 + math.exp((-1) * FALLING_STREAK_CONSTANT * abs(streakB))))
        else:
            STREAK_FACTOR_B = 1
    elif scoreB > scoreA:
        if streakA < 0:
            STREAK_FACTOR_A = (-0.5) * STREAK_LIMIT + (1.5 * STREAK_LIMIT) / (
                (1.0 + math.exp((-1) * FALLING_STREAK_CONSTANT * abs(streakA))))
        else:
            STREAK_FACTOR_A = 1
        if streakB >= 0:
            STREAK_FACTOR_B = (-0.5) * STREAK_LIMIT + (1.5 * STREAK_LIMIT) / (
                (1.0 + math.exp((-1) * RISING_STREAK_CONSTANT * abs(streakB))))
        else:
            STREAK_FACTOR_B = 1
    else:
        STREAK_FACTOR_A = 1
...
def standard_team(team):
    ...
    elos = [[[0 for i in range(CUR_YEAR - START_YEAR + 1)] for i in range(WEEKS_PER_YEAR + 1)] for i in
            range(len(nflgame.teams) - 1)]
    ...
    for i in nflgame.teams:
        ...
        for i in range(START_YEAR, CUR_YEAR + 1):
                    ...
                    if not (i == CUR_YEAR and j > CUR_WEEK):
                        ...
                        if len(games) > 0:
                            for game in games:
                                ...
                                elos[hteam][j][i - START_YEAR], elos[ateam][j][i - START_YEAR] = eloEval(
                                    elos[hteam][j - 1][i - START_YEAR], elos[ateam][j - 1][i - START_YEAR],
                                    game.score_home,
                                    game.score_away, streaks[hteam], streaks[ateam])
            ...
            if i != CUR_YEAR:
                for x in range(len(nflgame.teams) - 1):
                    elos[x][0][i - START_YEAR + 1] = (SEASON_RESET) * elos[x][WEEKS_PER_YEAR][i - START_YEAR] + (
                            1 - SEASON_RESET) * INIT_ELO
